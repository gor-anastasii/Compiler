...
bool strwriteline(char word[]) {
	FST::FST strwriteline(word, 10,
		FST::NODE(1, FST::RELATION('w', 1)),
		FST::NODE(1, FST::RELATION('r', 2)),
		FST::NODE(1, FST::RELATION('i', 3)),
		FST::NODE(1, FST::RELATION('t', 4)),
	...
	return FST::execute(strwriteline);
}
...
bool literaluint2(char word[]) {
	FST::FST literaluint2(word, 4,
	...
	return FST::execute(literaluint2);
}
...
bool MathPow(char word[]) {
	FST::FST staticfi(word, 8,
		FST::NODE(1, FST::RELATION('M', 1)),
		FST::NODE(1, FST::RELATION('a', 2)),
		FST::NODE(1, FST::RELATION('t', 3)),
		FST::NODE(1, FST::RELATION('h', 4)),
		...
		FST::NODE(1, FST::RELATION('w', 7)),
		FST::NODE()
	);
	return FST::execute(staticfi);
}

bool MathRand(char word[]) {
	FST::FST staticfi(word, 9,
		FST::NODE(1, FST::RELATION('M', 1)),
		FST::NODE(1, FST::RELATION('a', 2)),
		FST::NODE(1, FST::RELATION('t', 3)),
		FST::NODE(1, FST::RELATION('h', 4)),
...
namespace LexAnal
{
	...
	Lex LexAnaliz(Parm::PARM parm, In::IN in)
	{
		...
		for (int i = 0; word[i] != NULL; i++)
		{
			...
			else if (strwriteline(word[i]))
			{
				LT::Entry entryLT = WriteEntry(entryLT, LEX_WRITELINE, LT_TI_NULLIDX, line);
				LT::Add(lextab, entryLT);
			}
			...
			else if (MathPow(word[i]) || MathRand(word[i])) {
				if (int idx = IT::IsId(idtab, word[i]) == TI_NULLIDX) {
					entryIT.idtype = IT::F;
					entryIT.iddatatype = IT::UINT;
					entryIT.idxfirstLE = indexLex;
					entryIT.params.count = 2;
					entryIT.params.type[entryIT.params.count - 2] = IT::UINT;
					entryIT.params.type[entryIT.params.count - 1] = IT::UINT;
			...
			{
				/*unsigned int value = atoi((char*)word[i]);*/
				unsigned long long int value = strtoul((const char*)word[i], NULL, 10);
				...
				numBuf[j] = '\0';
				unsigned int value = strtol(numBuf, NULL, 2);
				if (value > TI_UINT_MAXSIZE)
				{
					throw ERROR_THROW_IN(313, line, position);
				}
			...
			else if (literaluint16(word[i])) {
				char* numBuf = new char;
				int j = 0;
				for (int k = 3; k < strlen(word[i]); k++, j++)
				{
					numBuf[j] = word[i][k];
				}
				numBuf[j] = '\0';
				unsigned long int value = strtoul(numBuf, NULL, 16);
				/*unsigned int value = strtol(numBuf, NULL, 16);*/
				if (value > TI_UINT_MAXSIZE)
				{
					throw ERROR_THROW_IN(313, line, position);
				}
			...
			else if (idenf(word[i]))
			{
				...
				if (!findFunc && functions.empty()) throw ERROR_THROW_IN(303, line, position);;
				LT::Entry entryLT = WriteEntry(entryLT, LEX_ID, IT::IsId(idtab, word[i]), line);
				LT::Add(lextab, entryLT);
				entryIT = { };
			}
